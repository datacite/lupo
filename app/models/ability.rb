class Ability
  ...
  def initialize(user)
    ...
    if user.role_id == "staff_admin"
      ...
      can %i[update read read_billing_information], Provider, symbol: user.provider_id.upcase
      ...
      can %i[read get_url transfer read_landing_page_results], Doi, provider_id: user.provider_id
      can [:read], Doi
      can [:read], User
      can [:read], Phrase
      can [:read], Activity do |activity|
      ...
      can %i[read read_billing_information], Provider, symbol: user.provider_id.upcase
      can [:read], Provider
      ...
      can %i[read get_url read_landing_page_results], Doi, provider_id: user.provider_id
      can [:read], Doi
      can [:read], User
      can [:read], Phrase
      can [:read], Activity do |activity|
      ...
      can %i[read destroy update register_url validate undo get_url get_urls read_landing_page_results], Doi, client_id: user.client_id
      ...
      can [:read], ClientPrefix, client_id: user.client_id
      can %i[read get_url read_landing_page_results], Doi, client_id: user.client_id
      can [:read], Doi
      can [:read], User
      can [:read], Phrase
      can [:read], Activity do |activity|
      ...
      can %i[read update], Client, symbol: user.client_id.upcase if user.client_id.present?
