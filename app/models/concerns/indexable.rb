  ...
  included do
    after_commit on: %i[create update] do
      ...
      # ignore if record was created via Salesforce API
        ...
        puts "--------------------------------------------"
        puts "GOT HERE - EXPORTING CONTACT - indexable:58 - #{email} - provider: #{provider.name}"
        ...
        provider.contacts.each do |c|
          ...
          puts "STILL SENDING CONTACT EXPORT MESSAGE FOR CONTACT: #{c.email} - #{c.provider.name}"
          send_contact_export_message(c.to_jsonapi.merge(slack_output: true))
        end
        puts "SENDING CONTACT EXPORT MESSAGE FOR CONTACT: #{email} - #{provider.name}"
