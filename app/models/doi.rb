...
class Doi < ApplicationRecord
  ...
  def self.import_by_ids(options = {})
    ...
    if options[:model] = "DataciteDoi"
      ...
      collection = DataciteDoi.where(datacentre: client.id)
    else
      collection = Doi
    end

    from_id =
      (options[:from_id] || collection.minimum(:id)).
        to_i
    until_id =
      (
        options[:until_id] ||
        collection.maximum(:id)
      ).
        to_i
  ...
  def self.import_by_id(options = {})
    ...
    collection.where(id: id..(id + 499)).
      ...
      response =
        Doi.__elasticsearch__.client.bulk index: index,
                                                  type:
                                                    Doi.document_type,
                                                  body:
                                                    dois.map { |doi|
