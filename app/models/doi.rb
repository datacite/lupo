...
class Doi < ApplicationRecord
  ...
  validates_format_of :url, with: /\A(ftp|http|https):\/\/\S+\z/, if: Proc.new { |doi| doi.url? && !doi.skip_url_validation }, message: "URL is not valid"
  ...
  validates :geo_locations, if: proc { |doi| doi.validate_json_attribute?(:geolocations) }, json: { message: ->(errors) { errors }, schema: lambda { schema_file_path("geolocations") } }, unless: :only_validate
  ...
  def self.import_one(doi_id: nil, id: nil)
    ...
    attrs = attrs.merge(skip_url_validation: true, skip_schema_version_validation: true)
    doi.update(attrs)
