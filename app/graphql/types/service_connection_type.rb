...
class ServiceConnectionType < BaseConnection
  ...
  def response(**args)
    @response ||= Doi.query(args[:query], user_id: args[:user_id], client_id: args[:repository_id], provider_id: args[:member_id], year: args[:year], resource_type_id: "Service", has_citations: args[:has_citations], has_parts: args[:has_parts], has_versions: args[:has_versions], has_views: args[:has_views], has_downloads: args[:has_downloads], page: { number: 1, size: 0 })
  end
end
