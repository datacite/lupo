...
class ProviderPrefixConnectionWithMetaType < BaseConnection
  edge_type(ProviderPrefixEdgeType)
  ...
  def response(**args)
    @response ||= ProviderPrefix.query(args[:query], provider_id: args[:provider_id], state: args[:state], year: args[:year], page: { number: 1, size: 0 })
  end
end
