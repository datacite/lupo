class EventsQuery
  ...
  def doi_citations(doi)
    ...
    results = Event.query(query, aggregations: "citation_count_aggregation", page: { size: 1, cursor: [] }).response.aggregations.citations.buckets
  ...
  def citations(doi)
    doi.downcase.split(",").map do |item|
      {id: item, total: EventsQuery.new.doi_citations(item)}
    end
  end
