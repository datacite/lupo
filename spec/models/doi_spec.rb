...
describe Doi, type: :model, vcr: true do
  ...
  it "dois" do
    dois = Doi.where(rows: 60)
    expect(dois[:data].length).to eq(60)
    doi = dois[:data].last
    expect(doi.title).to eq("IMG_0134.jpg")
    expect(doi.resource_type.title).to eq("Dataset")
    meta = dois[:meta]
  ...
  it "dois with query" do
    dois = Doi.where(query: "cancer")
    expect(dois[:data].length).to eq(25)
    doi = dois[:data].first
    expect(doi.title).to eq("Cooking the Books: the Golem and the Ethics of Biotechnology")
    expect(doi.resource_type.title).to eq("Text")
  end

  it "dois with query sort by minted" do
    dois = Doi.where(query: "cancer", sort: "minted")
    expect(dois[:data].length).to eq(25)
    doi = dois[:data].first
    expect(doi.title).to eq("Cooking the Books: the Golem and the Ethics of Biotechnology")
    expect(doi.resource_type.title).to eq("Text")
  ...
  it "dois with query and resource-type-id" do
    dois = Doi.where(query: "cancer", "resource-type-id" => "dataset")
    expect(dois[:data].length).to eq(3)
    doi = dois[:data].first
    expect(doi.title).to eq("Landings of European lobster (Homarus gammarus) and edible crab (Cancer pagurus) in 2011, Helgoland, North Sea")
    expect(doi.resource_type.title).to eq("Dataset")
  end

  it "dois with query and resource-type-id and data-center-id" do
    dois = Doi.where(query: "cancer", "resource-type-id" => "dataset", "data-center-id" => "FIGSHARE.ARS")
    expect(dois[:data].length).to eq(25)
    doi = dois[:data].first
    expect(doi.title).to eq("Achilles_v3.3.7_README.txt")
    expect(doi.resource_type.title).to eq("Dataset")
  end

  it "doi" do
    doi = Doi.where(id: "10.3886/ICPSR36357.V1")[:data]
    expect(doi.title).to eq("Arts and Cultural Production Satellite Account")
    expect(doi.resource_type.title).to eq("Dataset")
    expect(doi.data_center.title).to eq("ICPSR")
  end
end
