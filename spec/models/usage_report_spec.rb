...
describe UsageReport, type: :model, vcr: true do
  describe "find_by_id" do
    it "found" do
      ...
      expect(usage_reports[:data].first).to eq(
        id:
          "https://api.test.datacite.org/reports/0498876e-dd55-42b0-b2a6-850df004a0e4",
        reporting_period: { begin_date: "2018-10-01", end_date: "2018-10-31" },
      )
    end
  ...
  describe "query" do
    it "all" do
      ...
      expect(usage_reports.dig(:meta, "total")).to eq(309)
      expect(usage_reports[:data].size).to eq(25)
      expect(usage_reports[:data].first).to eq(
        id:
          "https://api.test.datacite.org/reports/0148536f-bfbc-4775-b93c-524e91d0b9f6",
