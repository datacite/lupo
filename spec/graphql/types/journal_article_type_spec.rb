...
describe JournalArticleType do
  ...
  describe "query journal articles", elasticsearch: true do
    let!(:journal_articles) do
      create_list(
        :doi,
        3,
        types: {
          "resourceTypeGeneral" => "Text", "resourceType" => "JournalArticle"
        },
        aasm_state: "findable",
      )
    end
    ...
    it "returns all journal articles" do
      ...
      expect(response.dig("data", "journalArticles", "nodes", 0, "id")).to eq(
  ...
  describe "query journal articles by person", elasticsearch: true do
    ...
    it "returns journal articles" do
      ...
      expect(response.dig("data", "journalArticles", "totalCount")).to eq(3)
      expect(response.dig("data", "journalArticles", "published")).to eq(
