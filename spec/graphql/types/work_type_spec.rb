...
describe WorkType do
  ...
  describe "find work", elasticsearch: true do
    let!(:work) do
      create(:doi, aasm_state: "findable", container:
      { "type" => "Journal",
        "issue" => "9",
  ...
  describe "find work crossref", elasticsearch: true, vcr: true do
    let!(:work) do
      create(:doi, doi: "10.1038/nature12373", agency: "crossref", aasm_state: "findable", titles: [
  ...
  describe "query works", elasticsearch: true, vcr: true do
    ...
    let!(:works) do
      create_list(:doi, 10, aasm_state: "findable", client: client_without_ror, creators:
      [{
        "familyName" => "Garza",
        "givenName" => "Kristian",
        "name" => "Garza, Kristian",
    ...
    let!(:doi) do
      create(:doi, aasm_state: "findable", client: client_without_ror, creators:
      [{
        "familyName" => "Garza",
        "givenName" => "Kristian",
        "name" => "Garza, Kristian",
    ...
    let!(:organization_doi) do
      ...
      [{
        ...
        "nameIdentifiers" => [{ "nameIdentifier" => "https://ror.org/013meh722", "nameIdentifierScheme" => "ROR", "schemeUri" => "https://ror.org" }],
        "nameType" => "Organizational",
      }])
    end
    let!(:funder_doi) do
      create(:doi, aasm_state: "findable", client: client_without_ror, funding_references:
      [{
    ...
    it "returns all works" do
      ...
      expect(response.dig("data", "works", "totalCount")).to eq(14)
      expect(Base64.urlsafe_decode64(response.dig("data", "works", "pageInfo", "endCursor")).split(",", 2).last).to eq(@works[11].uid)
      expect(response.dig("data", "works", "pageInfo", "hasNextPage")).to be true
      expect(response.dig("data", "works", "nodes").length).to eq(4)
      ...
      expect(response.dig("data", "associated", "published")).to eq([{ "count" => 4, "id" => "2011", "title" => "2011" }])
      expect(response.dig("data", "contributed", "totalCount")).to eq(1)
      expect(response.dig("data", "contributed", "published")).to eq([{ "count" => 1, "id" => "2011", "title" => "2011" }])
      expect(response.dig("data", "affiliated", "totalCount")).to eq(3)
      expect(response.dig("data", "affiliated", "published")).to eq([{ "count" => 3, "id" => "2011", "title" => "2011" }])
      expect(response.dig("data", "funded", "totalCount")).to eq(1)
      expect(response.dig("data", "funded", "published")).to eq([{ "count" => 1, "id" => "2011", "title" => "2011" }])
      expect(response.dig("data", "hosted", "totalCount")).to eq(1)
      expect(response.dig("data", "hosted", "published")).to eq([{ "count" => 1, "id" => "2011", "title" => "2011" }])
    end
  end
  ...
  describe "query works by license", elasticsearch: true do
    ...
    let!(:works) do
      create_list(:doi, 10, aasm_state: "findable", agency: "datacite", subjects:
      [{
