  ...
  describe 'GET /dois all with metrics', elasticsearch: true, vcr: true do
    ...
    context 'when the record exists' do
      it 'returns the Doi' do
        ...
        expect(result.dig('attributes', 'url')).to eq(doi.url)
        expect(result.dig('attributes', 'doi')).to eq(doi.doi.downcase)
        expect(result.dig('attributes', 'titles')).to eq(doi.titles)
        expect(result.dig('attributes','citations')).to eq(3)
        expect(result.dig('attributes','views')).to be > 0
        expect(result.dig('attributes','downloads')).to eq(0)
      end
    end
  end
