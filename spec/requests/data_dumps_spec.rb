...
describe DataDumpsController, type: :request, elasticsearch: true do
  ...
  let(:bad_headers) { { "HTTP_ACCEPT" => "application/vnd.api+json", "HTTP_AUTHORIZATION" => "Basic 12345" } }
  let(:user_token) { User.generate_token(role_id: "user") }
  let(:user_headers) { { "HTTP_ACCEPT" => "application/vnd.api+json", "HTTP_AUTHORIZATION" => "Bearer " + user_token } }
  ...
  describe "GET /data_dumps", elasticsearch: true do
    ...
    context "with valid authorization" do
      ...
      it "returns data dumps with pagination" do
        get "/data_dumps?page[number]=1&page[size]=4", nil, headers
    ...
    context "with bad authorization" do
      it "returns access denied" do
        get "/data_dumps", nil, bad_headers
        expect(last_response.status).to eq(401)
      end
    end
    ...
    context "with insufficient permission" do
      it "returns access denied" do
        get "/data_dumps", nil, user_headers
