  ...
  describe 'POST /metadata' do
    ...
    context 'when the doi is missing' do
      ...
      it 'returns a validation failure message' do
        expect(json).to eq("errors"=>[{"id"=>"doi", "title"=>"Doi must exist"}])
      end
    end
    ...
    context 'when the XML is not valid findable status' do
      ...
      it 'returns a validation failure message' do
        expect(json).to eq("errors"=>[{"id"=>"xml", "title"=>"Xml 4:0: ERROR: Element '{http://datacite.org/schema/kernel-4}creators': Missing child element(s). Expected is ( {http://datacite.org/schema/kernel-4}creator )."}])
      end
    end
  end
