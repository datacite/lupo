...
RSpec.describe "Datasets", type: :request do
  ...
  let(:dataset_id) { datasets.first.id }
  ...
  describe 'GET /datasets/:id' do
    before { get "/datasets/#{dataset_id}" }

    context 'when the record exists' do
      it 'returns the Dataset' do
        expect(json).not_to be_empty
        expect(json['data']['id']).to eq(dataset_id)
      end
  ...
  describe 'POST /datasets' do
    ...
    context 'when the request is valid' do
      before { post '/datasets', params: valid_attributes, headers: {'HTTP_ACCESS'=>'application/vnd.api+json', 'CONTENT_TYPE'=>'application/vnd.api+json'} }
    ...
    context 'when the request is invalid' do
      before { post '/datasets', params: { datacentre: 222 } }
  ...
  #
  #   context 'when the record exists' do
  #     before { put "/datasets/#{dataset_id}", params: valid_attributes }
  #
  ...
  describe 'DELETE /datasets/:id' do
    before { delete "/datasets/#{dataset_id}" }
