...
describe EventsController, type: :request, elasticsearch: true, vcr: true do
  ...
  context "update" do
    ...
    let(:uri) { "/v3/events/#{event.uuid}?include=dois" }
  ...
  context "show" do
    ...
    let(:uri) { "/v3/events/#{event.uuid}?include=doi-for-source,doi-for-target" }
    ...
    context "event not found" do
      let(:uri) { "/v3/events/#{event.uuid}x" }
  ...
  context "index" do
    ...
    context "query by source-id by Crawler" do
      let(:uri) { "/v3/events?query=datacite" }
