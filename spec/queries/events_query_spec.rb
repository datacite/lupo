...
describe EventsQuery, elasticsearch: true do
  ...
  context "usage events" do
    ...
    it "usage" do
      response = EventsQuery.new.views_and_downloads("10.0260/co.2004960.v1").first
      expect(response[:downloads]).to be > 0
      expect(response[:views]).to be > 0
    end
  end
  ...
  context "mutiple usage events" do
    ...
    it "show views" do
      response = EventsQuery.new.views(views.map(&:doi).join(","))
      # expect(response.size).to eq(5)
      expect(response.first[:views]).to be > 0
    end

    it "show downloads" do
      response = EventsQuery.new.downloads(downloads.map(&:doi).join(","))
      # expect(response.size).to eq(5)
