...
Mime::Type.register "application/json",
                    :json,
                    %w[
                      text/x-json
...
Mime::Type.register "application/vnd.datacite.datacite+xml",
                    :datacite,
                    %w[application/x-datacite+xml]
Mime::Type.register "application/vnd.datacite.datacite+json", :datacite_json
Mime::Type.register "application/vnd.schemaorg.ld+json",
                    :schema_org,
                    %w[application/ld+json]
Mime::Type.register "application/vnd.jats+xml", :jats
Mime::Type.register "application/vnd.citationstyles.csl+json",
                    :citeproc,
                    %w[application/citeproc+json]
...
ActionController::Renderers.add :datacite do |obj, _options|
  Array.wrap(obj).map(&:xml).join("\n")
end
...
%w[datacite_json schema_org crosscite citeproc codemeta].each do |f|
  ActionController::Renderers.add f.to_sym do |obj, _options|
    if obj.is_a?(Array)
...
ActionController::Renderers.add :bibtex do |obj, _options|
  Array.wrap(obj).map { |o| o.send("bibtex") }.join("\n")
end

ActionController::Renderers.add :ris do |obj, _options|
