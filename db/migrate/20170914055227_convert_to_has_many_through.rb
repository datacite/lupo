...
class ConvertToHasManyThrough < ActiveRecord::Migration[5.1]
  def up
    safety_assured { execute "ALTER TABLE `allocator_prefixes` DROP PRIMARY KEY" }
    ...
    safety_assured { execute "ALTER TABLE `datacentre_prefixes` DROP PRIMARY KEY" }
  ...
  def down
    ...
    safety_assured { execute "ALTER TABLE `allocator_prefixes` ADD PRIMARY KEY (allocator,prefixes);" }
    ...
    safety_assured { execute "ALTER TABLE `datacentre_prefixes` ADD PRIMARY KEY (datacentre,prefixes);" }
  end
end
