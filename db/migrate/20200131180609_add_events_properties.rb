...
class AddEventsProperties < ActiveRecord::Migration[5.2]
  def up
    ...
    add_index :events, %i[source_doi source_relation_type_id], name: "index_events_on_source_doi", length: { source_doi: 100 }
    add_index :events, %i[target_doi target_relation_type_id], name: "index_events_on_target_doi", length: { target_doi: 100 }
  end
