...
- Run the API in development: `bundle exec rails server -b 0.0.0.0 -p 3000`.
  Production serves via Passenger inside Docker, so Puma is declared only in the
  `:development` group of the `Gemfile` to provide a native dev server.
